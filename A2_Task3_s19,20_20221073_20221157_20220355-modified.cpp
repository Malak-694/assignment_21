...
public:
    explicit CPU(MEMORY& member) : memory(member), registers(16, 0), pc(0) {}
        ...
        switch (opcode){
            ...
            case '3':
                if (operand1 == '0') {
                    ...
                    tmp += operand2;
                    tmp += operand3;
                    int memAddress = decode(tmp);
                    cout<<tmp<<" "<<memAddress<<" "<<registers[operand1 - '0']<<"\n";
                    memory.write_val(memAddress, to_string(registers[operand1 - '0']));
                }
...
int main() {
    ...
    CPU cpu(memory);
    ifstream inFile;
    cout<<"Enter the name of the file you want to simulate: ";
    string name;
    cin>>name;
    inFile.open(name.c_str());
    //inFile.open("C:\\Users\\Malak\\Desktop\\A2_Malak Ahmed, Salma gamal , Nadra Mahmoud_20221157,2022335,20221073\\A2\\si.txt.txt");
    ...
    while(answer==1) {
        ...
        cpu.print_me();
        cout<<"\n "
              "(1) Do you want to execute a file" <<" OR "<< "(2) End the program ? ";
        cin>>answer;
    }
